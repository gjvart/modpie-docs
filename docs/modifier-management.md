...
```
 [▼] [Bevel]         [👁] [✏] [Render] [Cage]  [⚙]  [📌]  |  [✓] [✕]
      │                                         │    │          │
    Toggle                                     Gear  Pin      Apply &
   Collapse                                   Tools Badge     Remove
```
...
- <kbd>▲</kbd> **Move Up**: Shifts the modifier one position upward toward the top of the stack (earlier evaluation).
- <kbd>▼</kbd> **Move Down**: Shifts the modifier one position downward toward the bottom of the stack (later evaluation).
- <kbd>📌</kbd> **Pin to Last**: Pins or unpins the modifier to the very bottom of the stack.
- <kbd>✏</kbd> **Rename**: Opens an instant popup prompt to rename the modifier (see [Inline Modifier Renaming](inline-renaming.md)).
...
The **Pin to Last** (<kbd>📌</kbd>) system ensures critical terminal modifiers (like **Subdivision Surface**, **Bevel**, **Weighted Normal**, or **Triangulate**) stay locked at the bottom of the evaluation order:
...
- **Closed Modifier Pin Badge**: When a pinned modifier is collapsed with its gear menu closed, a highlighted <kbd>📌</kbd> badge remains permanently visible directly on the outer header row. You can immediately identify which modifier is pinned at a glance.
...
- A visual separator precedes the **Apply** (<kbd>✓</kbd>) and **Delete** (<kbd>✕</kbd>) buttons on the right side of the card header.
...
| :--- | :---: | :---: |
| **Collapsed 4-Switch Visibility Toggles** | ✔ | ✔ |
| **Quick Gear Tools (`⚙`) (Move Up / Down)** | ✔ | ✔ |
| **Dynamic Pin to Last Embossing & Alerts** | ✔ | ✔ |
| **Visible Pin Badge on Collapsed Cards** | ✔ | ✔ |
| **Grouped Apply & Delete Buttons** | ✔ | ✔ |
